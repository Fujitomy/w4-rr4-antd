...
module.exports = {
    ...
    output: {
        ...
        // 输出文件名 [name].dll.[chunkhash].js 中 [name] 对应上文的entry.vendor的生成的动态链接库文件名为vendor.dll.[chunkhash].js
        filename: '[name].dll.[chunkhash].js',
        // 动态链接库文件内的全局变量名称命名（映射关系），与下文DllPlugin.name对应
    ...
    plugins:[
        ...
        new webpack.DllPlugin({
            // context是解析包路径的上下文
            context: __dirname,
            // name是dll內部暴露的对象名(manifest清单索引文件内的name),与output.library保持一致
            name: '_dll_[name]_[chunkhash]',
            // TODO 生成manifest索引清单的文件名，竟然是path属性，这里更改manifest的文件名，why not filename？
